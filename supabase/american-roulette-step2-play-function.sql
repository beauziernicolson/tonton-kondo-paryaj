...
CREATE OR REPLACE FUNCTION public.play_american_roulette(
  p_user_id UUID,
  p_bets JSONB
)
RETURNS TABLE (
...
BEGIN
  ...
  LOOP
    ...
    IF left(v_bet_key, 2) = 'n:' THEN
      v_bet_number_text := substring(v_bet_key from 3);
      IF v_bet_number_text IS NULL OR length(v_bet_number_text) = 0 THEN
        RAISE EXCEPTION 'La mise exacte % est invalide.', v_bet_key;
      END IF;

      IF v_bet_number_text <> '00' AND v_bet_number_text !~ '^[0-9]+$' THEN
        RAISE EXCEPTION 'La mise exacte % est invalide.', v_bet_key;
      END IF;

      IF v_bet_number_text = '00' THEN
        NULL;
      ELSE
        v_bet_number := v_bet_number_text::int;
        IF v_bet_number < 0 OR v_bet_number > 36 THEN
          RAISE EXCEPTION 'La mise exacte % est invalide.', v_bet_key;
        END IF;
      END IF;
    END IF;
  ...
  IF v_wallet_balance < v_total_bet_amount THEN
    RAISE EXCEPTION 'Solde insuffisant pour jouer à American Roulette.';
  END IF;

  v_rand := floor(random() * 38)::int;
  v_winning_slot := CASE v_rand
    WHEN 0 THEN '0'
    WHEN 1 THEN '00'
    ELSE (v_rand - 1)::text
  END;

  IF v_winning_slot IN ('0','00') THEN
    v_winning_color := 'green';
  ELSIF v_winning_slot IN (
  ...
  CREATE TEMP TABLE temp_american_roulette_bet_results (
    bet_key TEXT,
    bet_label TEXT,
    bet_type TEXT,
    bet_value TEXT,
  ...
  LOOP
    ...
    IF left(v_bet_key, 2) = 'n:' THEN
      ...
      IF v_bet_value_text = v_winning_slot THEN
        v_outcome := 'won';
    ...
    ELSIF v_bet_key = 'even' THEN
      v_payout_multiplier := 2;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int % 2 = 0) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 2;
      END IF;
    ELSIF v_bet_key = 'odd' THEN
      v_payout_multiplier := 2;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int % 2 = 1) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 2;
      END IF;
    ELSIF v_bet_key = 'low' THEN
      v_payout_multiplier := 2;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int BETWEEN 1 AND 18) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 2;
      END IF;
    ELSIF v_bet_key = 'high' THEN
      v_payout_multiplier := 2;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int BETWEEN 19 AND 36) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 2;
      END IF;
    ELSIF v_bet_key = 'dozen1' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int BETWEEN 1 AND 12) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    ELSIF v_bet_key = 'dozen2' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int BETWEEN 13 AND 24) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    ELSIF v_bet_key = 'dozen3' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int BETWEEN 25 AND 36) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    ELSIF v_bet_key = 'col1' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int % 3 = 1) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    ELSIF v_bet_key = 'col2' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int % 3 = 2) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    ELSIF v_bet_key = 'col3' THEN
      v_payout_multiplier := 3;
      IF v_winning_slot NOT IN ('0','00') AND (v_winning_slot::int % 3 = 0) THEN
        v_outcome := 'won';
        v_win_amount := v_amount * 3;
      END IF;
    END IF;
    ...
    INSERT INTO temp_american_roulette_bet_results (
      bet_key,
      bet_label,
      bet_type,
      bet_value,
  ...
  v_ticket_number := 'TKAR-' || to_char(NOW(), 'YYYYMMDD-HH24MISS') || '-' || substring(md5(random()::text) FROM 1 FOR 6);
  ...
  FROM temp_american_roulette_bet_results;

  PERFORM public.apply_transaction(
    v_user_id,
    'bet',
    v_total_bet_amount,
    'bet-american-roulette-' || v_ticket_number,
    'Mise American Roulette',
    jsonb_build_object(
      'game_type', 'american_roulette',
  ...
  IF v_total_win_amount > 0 THEN
    PERFORM public.apply_transaction(
      ...
      'win',
      v_total_win_amount,
      'win-american-roulette-' || v_ticket_number,
      'Gain American Roulette',
      jsonb_build_object(
        'game_type', 'american_roulette',
        'ticket_number', v_ticket_number,
        'bets', v_bets_obj,
  ...
  INSERT INTO public.american_roulette_rounds (
    user_id,
    ticket_id,
    winning_slot,
    winning_color,
  ...
  INSERT INTO public.american_roulette_bets (
    round_id,
    user_id,
    ticket_id,
    bet_key,
...
COMMENT ON FUNCTION public.play_american_roulette(uuid, jsonb) IS
 'Joue un tour de American Roulette, crée le ticket et le round correspondants,
  enregistre chaque mise et applique les transactions via public.apply_transaction().' ;
